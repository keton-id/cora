...
pub const MemStore = struct {
    ...
    map: std.StringHashMapUnmanaged(*SecretBuf) = .{},
    ...
    pub fn put(self: *MemStore, name: []const u8, value: []const u8) !void {
        ...
        if (gop.found_existing) {
            gop.value_ptr.*.zero();
            try gop.value_ptr.*.set(value);
            return;
        }
        const key_copy = try self.allocator.dupe(u8, name);
        errdefer self.allocator.free(key_copy);
        const buf = try self.allocator.create(SecretBuf);
